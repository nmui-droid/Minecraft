...
log_texture  = load_texture('assets/log_block.png')
...
class Voxel(Button):
	def __init__(self, position = (100,100,100), texture = stone_texture):
		super().__init__(
			parent = scene,
			position = position,
			model = 'assets/block',
			origin_y = 0.5,
			texture = texture,
			color = color.color(0,0,random.uniform(0.9,9)),
			scale = 0.5)
